...
export function demoTutor(messages: Message[]): string {
  ...
  const sentence = normalizeAnswer(
    input
      .replace(
        /^(correct my (sentence|english)|check my paragraph|rewrite correctly)\s*:\s*/i,
        "",
      )
      .replace(/^[“"‘]|[”"’]$/g, ""),
  );
  const match = questions.find(
    (q) =>
      q.category === "Correct the Mistake" &&
      normalizeAnswer(q.question.replace(/^Rewrite correctly:\s*/, "")).replace(
        /[‘’]/g,
        "'",
      ) === sentence,
  );
  ...
  const previous = messages
    ...
    .at(-1)?.content;
  if (
    /speaking|conversation|talk to me|friend|hobbies/.test(lower) ||
    previous?.startsWith(speakingIntro)
  ) {
    const topic =
      speakingTopics.find((t) => lower.includes(t.name.toLowerCase())) ||
      speakingTopics.find((t) =>
        messages.some((m) => m.content.includes(`${speakingIntro}: ${t.name}`)),
      ) ||
      speakingTopics[2];
    const index = previous?.startsWith(speakingIntro)
      ? messages.filter(
          (m) =>
            m.role === "assistant" &&
            m.content.startsWith(`${speakingIntro}: ${topic.name}`),
