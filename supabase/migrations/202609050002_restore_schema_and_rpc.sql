-- Recovery migration for an empty, partially initialized, or fully initialized project.
-- No rows are inserted, updated, or deleted. Existing records are preserved.
-- Run the entire file as the database owner in Supabase SQL Editor.
-- Re-running is safe; an incompatible existing schema causes an atomic failure.
begin;
...
create table if not exists public.ai_usage (id bigint generated always as identity primary key,session_id uuid not null references public.anonymous_sessions(session_id) on delete cascade,created_at timestamptz not null default now());
...
create table if not exists public.ai_budgets (bucket text primary key, requests integer not null default 0, expires_at timestamptz not null);

-- Fail explicitly when manually created tables do not match the app contract.
do $schema_check$
declare item record;
begin
 for item in
  select expected.table_name, expected.column_name, expected.type_name
  from (values
   ('anonymous_sessions','session_id','uuid'),
   ('anonymous_sessions','created_at','timestamp with time zone'),
   ('anonymous_sessions','last_seen_at','timestamp with time zone'),
   ('quiz_sessions','id','uuid'), ('quiz_sessions','session_id','uuid'),
   ('quiz_sessions','level','text'), ('quiz_sessions','category','text'),
   ('quiz_sessions','total_questions','integer'), ('quiz_sessions','correct_answers','integer'),
   ('quiz_sessions','score_percentage','numeric'), ('quiz_sessions','completed_at','timestamp with time zone'),
   ('quiz_attempts','id','bigint'), ('quiz_attempts','run_id','uuid'), ('quiz_attempts','session_id','uuid'),
   ('quiz_attempts','question_id','text'), ('quiz_attempts','category','text'),
   ('quiz_attempts','is_correct','boolean'), ('quiz_attempts','created_at','timestamp with time zone'),
   ('assessments','id','uuid'), ('assessments','session_id','uuid'), ('assessments','assessment_type','text'),
   ('assessments','version','text'), ('assessments','vocabulary_score','numeric'),
   ('assessments','grammar_score','numeric'), ('assessments','speaking_score','numeric'),
   ('assessments','writing_score','numeric'), ('assessments','total_score','numeric'),
   ('assessments','created_at','timestamp with time zone'),
   ('ai_usage','id','bigint'), ('ai_usage','session_id','uuid'), ('ai_usage','created_at','timestamp with time zone'),
   ('activity_events','id','bigint'), ('activity_events','session_id','uuid'),
   ('activity_events','name','text'), ('activity_events','detail','text'), ('activity_events','created_at','timestamp with time zone'),
   ('ai_budgets','bucket','text'), ('ai_budgets','requests','integer'), ('ai_budgets','expires_at','timestamp with time zone')
  ) as expected(table_name,column_name,type_name)
 loop
  if not exists (
   select 1 from information_schema.columns c where c.table_schema='public'
   and c.table_name=item.table_name and c.column_name=item.column_name and c.data_type=item.type_name
  ) then raise exception 'Incompatible schema: public.%.% must have type %. No changes committed.',item.table_name,item.column_name,item.type_name;
  end if;
 end loop;
end;
$schema_check$;

create index if not exists quiz_sessions_session_time on public.quiz_sessions(session_id,completed_at);
create index if not exists quiz_attempts_session on public.quiz_attempts(session_id);
create index if not exists assessments_session_version on public.assessments(session_id,version);
create index if not exists ai_usage_session_time on public.ai_usage(session_id,created_at);
create index if not exists activity_events_name_time on public.activity_events(name,created_at);
create index if not exists activity_events_session on public.activity_events(session_id);
...
create or replace function public.save_quiz(p_run uuid,p_session uuid,p_level text,p_category text,p_total integer,p_correct integer,p_score numeric,p_attempts jsonb) returns void
...
create or replace function public.save_assessment(p_session uuid,p_type text,p_version text,p_vocabulary numeric,p_grammar numeric,p_speaking numeric,p_writing numeric,p_total numeric) returns jsonb
language plpgsql security invoker set search_path=public as $$
...
create or replace function public.reserve_ai_request(p_session uuid) returns boolean
language plpgsql security invoker set search_path=public as $$
...
create or replace function public.research_summary() returns jsonb
...
-- Delivered only after commit; refresh PostgREST's visible function signatures.
