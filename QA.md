...
## Visual and UX upgrade with shadcn/ui

- Replaced the shared visual system across all eight routes: self-hosted Manrope/Noto Sans typography, forest-green accents, consistent cards and fields, desktop sidebar, mobile navigation sheet, clearer learning/practice layouts, compact chat composer, prompt search, and readable research results. The actual shadcn/ui Radix components are checked into `src/components/kit`.
- Inspected the home, learning, practice, prompts, assistant, speaking, research, and about screens in the browser. All eight routes had no horizontal document overflow at 390px in en/ru/kk, and at 320px, 768px, and 1440px in English. Visible buttons had accessible names.
- Mobile sheet: opening, focus trap (Shift+Tab wraps inside), Escape dismissal, and focus return to the menu trigger passed. Learning tabs switch both selection and the associated panel using ArrowRight.
- Prompt search found localized category text, returned a helpful zero-result state, and reset to the full library. Speaking's next-topic action showed five new questions, its copy action reported success, and the assistant continuation link populated the topic-specific English prompt.
- On a production copy at `localhost:3100` with Supabase disabled: completed an A2 grammar session with one correct and one incorrect answer (50%); checked the explicit unsaved notice. Canceling restart and switching to Kazakh preserved the selected answer. The incorrect choice now has a distinct warm border and cross; the correction has a check mark. New questions receive keyboard focus.
- The production demo chat returned the prepared correction. Canceling clear preserved both messages; confirming clear removed them and returned focus to the composer. Confirmations are translated, use modal focus handling, and keep actions at a comfortable touch size. Enter submission ignores active IME composition.
- On the connected app, opened the Before assessment, selected an answer locally, moved forward and back, canceled the exit dialog, then confirmed exit. The answer survived back navigation/cancel, and exit returned focus to the assessment overview. **No assessment was submitted or saved.** All completed practice QA used the separate unconfigured server; no synthetic research scores were written.
- The labeled Kazakh demo chart was visually checked at 390px. Multiword axis labels wrap, the legend and tooltip text have readable foreground colors, and the exact-value table remains available.
- Final checks passed: ESLint, strict TypeScript via the production build, all UI routes in `next build`, and the connected test suite (17 passed, one intentionally skipped). The isolated-server suite also passed 17 tests with the connected-only test skipped. The HTTP locale test covers all eight routes in each language and invalid-cookie fallback.
